...
describe('GET /auth/callback', () => {
  it('exchanges the code and redirects to `next`', async () => {
    ...
    const res = await callbackGET(
      new Request('http://localhost:5000/auth/callback?code=abc123&next=/tasks')
    )

    expect(mockExchange).toHaveBeenCalledWith('abc123')
    expect(res.status).toBe(307)
    expect(res.headers.get('location')).toBe('http://localhost:5000/tasks')
  })

  it('redirects to /login when there is no code', async () => {
    const res = await callbackGET(new Request('http://localhost:5000/auth/callback'))
...
describe('GET /auth/confirm', () => {
  it('verifies the OTP and redirects', async () => {
    ...
    const res = await confirmGET(
      new Request('http://localhost:5000/auth/confirm?token_hash=h&type=email&next=/')
    )

    expect(mockVerifyOtp).toHaveBeenCalledWith({ type: 'email', token_hash: 'h' })
    expect(res.headers.get('location')).toBe('http://localhost:5000/')
  })

  it('redirects to /login when params are missing', async () => {
    const res = await confirmGET(new Request('http://localhost:5000/auth/confirm'))
    expect(res.headers.get('location')).toContain('/login')
  })
})
...
describe('POST /auth/signout', () => {
  it('signs out and 303-redirects to /login', async () => {
    const res = await signoutPOST(
      new Request('http://localhost:5000/auth/signout', { method: 'POST' })
    )
