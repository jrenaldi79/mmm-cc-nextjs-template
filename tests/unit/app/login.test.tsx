...
describe('LoginPage', () => {
  ...
  it('shows an error message from searchParams', async () => {
    render(await LoginPage({ searchParams: Promise.resolve({ error: 'Invalid login credentials' }) }))
...
describe('SignupPage', () => {
  ...
  it('shows an error message from searchParams', async () => {
    render(await SignupPage({ searchParams: Promise.resolve({ error: 'User already registered' }) }))
